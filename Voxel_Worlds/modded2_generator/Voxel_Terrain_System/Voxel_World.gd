...
func _ready():
	...
	make_voxel_world(Vector3(8, 4, 8), Vector3(16, 16, 16));


func make_voxel_world(world_size, chunk_size):
	#
	# This would be a experimental map@@
	...
	# here we generate a random 2d map
	for i in range(100):
		var x = rand_range(1,255)
		var y = rand_range(1,255)
		var dis=rand_range(5,20)
		var ang=rand_range(0.0,PI*2.0)
		for j in range(dis):
			x+=cos(ang)*2
			y+=sin(ang)*2
			if(rand_range(0,20)<2):
				ang+=rand_range(0,PI*2.0)
			if(x>0 && y>0 && x<255 and y<255):
				genmap[x][y]=1
		
	# here we heighten and increase the dots size
	for i in range(256*256):
		var x = rand_range(5,250)
		var y = rand_range(5,250)
		if(genmap[x][y]>0 && genmap[x][y]<250):
			for x1 in range(-2,2):
				for y1 in range(-2,2):
					genmap[x+x1][y+y1]+=1
	...
	for i in range(256*256*5):
		var x = rand_range(2,253)			
		var y = rand_range(2,253)
		if(genmap[x][y]>1):
			var val=0
			for x1 in range(-1,2):
				for y1 in range(-1,2):
					val+=genmap[x+x1][y+y1]
			val/=9
			if(val>1):
				genmap[x][y]=val
	for y in range(256):
	...
	# here we copy the genmap into the world map
	for y in range(256):
		for x in range(256):			
				#for z in range(0,genmap[x][y]):
			if ( genmap[x][y]==0):
				genmap[x][y]=1;
			if(genmap[x][y]>255):
				genmap[x][y]=255
			for z in range(genmap[x][y],0,-1):
				if(z<10):
					#myworld[x][z][y]=get_voxel_int_from_string("Dirt");
					myworld[x][z][y]=get_voxel_int_from_string("Grass");
				if(z>=10):
					myworld[x][z][y]=get_voxel_int_from_string("Dirt");
				if(z>17):
					myworld[x][z][y]=0
	...
	for x in range(0, world_size.x):
		for y in range(0, world_size.y):
			for z in range(0, world_size.z):
				...
				new_chunk.setup(chunk_size.x, chunk_size.y, chunk_size.z, VOXEL_UNIT_SIZE,x*16,y*16,z*16);
