...
func splode():
	...
	#			set_cell(x,y,-1)
				...
				#at choice see if we can add some ore.
			...
			if(map[x][y]==0): #inside solid of asteroid
				...
				#
				# here we place the minerals(ore)
				#
				if(rand_range(0,10)<1):
					var zet=true
					for y1 in range(-1,2):
						for x1 in range(-1,2):
							if(map[x+x1][y+y1]==1):
								zet=false
					if(zet==true):
						var deepmine = randi()%10+1
						var tp=7+randi()%3
						set_cell(x-128,y-128,0,false,false,false,Vector2(tp,7))
						var zx = x
						var zy = y
						for i in range(deepmine):
							var ax = randi()%3-1
							var ay = randi()%3-1
							zx+=ax
							zy+=ay							
							if(map[zx-1][zy]==0 && map[zx+1][zy]==0 && map[zx][zy-1]==0 && map[zx][zy+1]==0):
								set_cell(zx-128,zy-128,0,false,false,false,Vector2(tp,7))
							else:
								i=10
				#
				# here we autotile the map
				...
				if(map[x+1][y]==1 && map[x-1][y]==0 && map[x][y-1]==0 && map[x][y+1]==0):
					...
					if(rand_range(0,10)<1):
						set_cell(x-128,y-128,0,false,false,false,Vector2(11,1))
					else:
				...
				if(map[x-1][y]==1 && map[x+1][y]==0 && map[x][y-1]==0 && map[x][y+1]==0):
					...
					if(rand_range(0,10)<1):
						set_cell(x-128,y-128,0,false,false,false,Vector2(9,1))
					else:
				...
				if(map[x][y-1]==1 && map[x][y+1]==0 && map[x-1][y]==0 && map[x+1][y]==0):
					...
					if(rand_range(0,10)<1):
						set_cell(x-128,y-128,0,false,false,false,Vector2(10,0))
					else:
				...
				if(map[x][y+1]==1 && map[x][y-1]==0 && map[x-1][y]==0 && map[x+1][y]==0):
					...
					if(rand_range(0,10)<1):
						set_cell(x-128,y-128,0,false,false,false,Vector2(10,2))
					else:
